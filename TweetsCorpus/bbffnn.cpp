...
//    cin >> LEARNING_RATE >> MAX_ERROR;
    ...
    tt.resize(total_cases);
    t.resize(total_cases);
            ...
            for(int i=0; i<total_cases; i++) {
                ...
                for(int j=0; j<theta[layers-1].size() ;j++) {
                    ...
                    for(int k=0; k<weights[layers-1][j].size(); k++) {
                        ...
                        weights[layers-1][j][k] += LEARNING_RATE*delta[layers-1][j]*output[layers-1][k]+MOMENTUM*dw[layers-1][j][k];
                        dw[layers-1][j][k]=LEARNING_RATE*delta[layers-1][j]*output[layers-1][k];
                    }
